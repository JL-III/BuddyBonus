...
public class Utils {
    public static Optional<Buddy> getOptionalBuddyFromBuddyList(List<Buddy> buddyList, UUID player_uuid) {
        ...
        for (Buddy buddy_in_list: buddyList) {
            if (buddy_in_list.player_one_UUID().equals(player_uuid)) { buddy = Optional.of(buddy_in_list); }
            if (buddy_in_list.player_two_UUID().equals(player_uuid)) { buddy = Optional.of(buddy_in_list); }
        }
        return buddy;
    }
    ...
    public static Optional<Request> getOptionalRequestFromRequestList(List<Request> requestList, UUID player_uuid) {
        ...
        for (Request request_in_list: requestList) {
            if (request_in_list.player_one_UUID().equals(player_uuid)) { request = Optional.of(request_in_list); }
            if (request_in_list.player_two_UUID().equals(player_uuid)) { request = Optional.of(request_in_list); }
        }
        return request;
    }
